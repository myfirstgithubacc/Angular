...
import { CommonComponentData } from '@xrm-core/models/location/location.model';
import { LocationService } from './services/location.service';

@Component({selector: 'app-core-location',
	templateUrl: './core-location.component.html',
	styleUrls: ['./core-location.component.scss'],
	changeDetection: ChangeDetectionStrategy.OnPush
})
export class CoreLocationComponent implements OnInit, OnDestroy {
	...
	private setStatusAndEventLogData(): void {
		this.locationService.sharedDataObservable.pipe(takeUntil(this.unsubscribeAll$)).subscribe((res: CommonComponentData) => {
			this.recordCode = res.LocationConfigCode;
			...
			this.setEventLogData(res.LocationConfigId);
		});
	}
	...
	private updateRecordStatus(requestData: IRecordStatusChangePayload[]): void {
		this.locationService.updateLocationStatus(requestData)
			.pipe(takeUntil(this.unsubscribeAll$))
			.subscribe(() => {
	...
	private getSuccessMessage(isDisabled: boolean): string {
		return isDisabled
			? 'LocationDeactivatedSuccessfully'
			: 'LocationActivatedSuccessfully';
	}
