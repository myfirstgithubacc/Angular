...
@Component({selector: 'app-core-organization-level2',
	templateUrl: './core-organization-level-2.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush
})
export class CoreOrganizationLevel2Component implements OnInit, OnDestroy {
	...
	public recordCodeLabel:string = OrgLevelCode.Two;
	...
	ngOnInit(): void {
		this.global.getRouteObs.pipe(switchMap((url:string) => {
			if(url.includes(NavigationUrls.Org2View)){
				this.isEditMode = false;
				this.isShowHeader = true;
			}else if(url.includes(NavigationUrls.Org2Edit)){
			...
			return this.organizationLevelService.OrgLevel2ParentData;
	...
	private onEdit = ():void => {
		this.router.navigate([`${NavigationUrls.Org2Edit}${this.uKey}`]);
	};
	...
	private updateRecord(data: StatusUpdatePayload[]):void {
		this.organizationLevelService.updateOrgLvl2BulkStatus(data)
