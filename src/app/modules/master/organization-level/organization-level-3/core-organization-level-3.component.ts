...
@Component({selector: 'app-core-organization-level3',
	templateUrl: './core-organization-level-3.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush
})
export class CoreOrganizationLevel3Component{
	...
	public recordCodeLabel:string = OrgLevelCode.Three;
	...
	ngOnInit(): void {
		this.global.getRouteObs.pipe(switchMap((url:string) => {
			if(url.includes(NavigationUrls.Org3View)){
				this.isEditMode = false;
				this.isShowHeader = true;
			}else if(url.includes(NavigationUrls.Org3Edit)){
			...
			return this.organizationLevelService.OrgLevel3ParentData;
	...
	private onEdit = ():void => {
		this.router.navigate([`${NavigationUrls.Org3Edit}${this.uKey}`]);
	};
	...
	private updateRecord(data: StatusUpdatePayload[]):void {
		this.organizationLevelService.updateOrgLvl3BulkStatus(data)
