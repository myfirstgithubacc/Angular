...
@Component({selector: 'app-core-organization-level4',
	templateUrl: './core-organization-level-4.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush
})
export class CoreOrganizationLevel4Component implements OnInit, OnDestroy{
	...
	public recordCodeLabel:string = OrgLevelCode.Four;
	private uKey:string = RequiredStrings.EmptyString;
	...
	ngOnInit(): void {
		this.global.getRouteObs.pipe(switchMap((url:string) => {
			if(url.includes(NavigationUrls.Org4View)){
				this.isEditMode = false;
				this.isShowHeader = true;
			}else if(url.includes(NavigationUrls.Org4Edit)){
			...
			return this.organizationLevelService.OrgLevel4ParentData;
		...
		this.statusData.items=[
			{
				...
				itemDynamicParam: [],
				cssClass: [RequiredStrings.BasicTitle],
				isLinkable: false,
	...
	private onEdit = ():void => {
		this.router.navigate([`${NavigationUrls.Org4Edit}${this.uKey}`]);
	};
	...
	private updateRecord(data: StatusUpdatePayload[]):void {
		this.organizationLevelService.updateOrgLvl4BulkStatus(data).pipe(takeUntil(this.unsubscribe$)).subscribe((res: GenericResponseBase<null>) => {
			if (res.Succeeded) {
