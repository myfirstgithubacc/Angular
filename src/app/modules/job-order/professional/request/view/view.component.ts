...
import { ILaborCategoryDetails, IProfPsrCandidateDetails, IProfRequest, IProfRequestAssignment, IProfRequestData, IProfRequestFinancial, IRequestComment } from '../../interface/shared-data.interface';
...
export class ProffViewComponent implements OnInit, OnDestroy{

	@Input() WidgetUkey: string;
	@Input() ParentEntity: string = ParentEntity.ProfessionalRequest;
	...
	public showAllSectionSwitch: boolean = false;
	...
	ngOnInit(): void {
		...
		if(this.ParentEntity == ParentEntity.ProfessionalRequest.toString()){
			...
			this.resetStep = true;
			this.getLiRequestDetails(this.WidgetUkey, StepperID.GetByUkey);
		}
	}
		...
		this.professionalRequestService.getReqViewById(uKey, stepperId).pipe(takeUntil(this.destroyAllSubscriptions$))
			.subscribe((res: GenericResponseBase<IProfRequestData>) => {
				if (res.Succeeded && res.Data) {
					...
					this.setPhonevalues(this.profRequest.ProfPsrCandidateDetails);
	...
	private setPhonevalues(candidateDetail: IProfPsrCandidateDetails) {
		this.showPhoneExt.controls['phoneControl'].setValue(candidateDetail.PhoneNumber);
		this.showPhoneExt.controls['phoneExt'].setValue(candidateDetail.PhoneExt);
	}
